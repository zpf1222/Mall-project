...
class SuperCat(db.Model):
    ...
    id = db.Column(db.Integer, unique=True)  # 编号
    cat_name = db.Column(db.String(100))  # 大分类名称
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    subcat = db.relationship("SuperCat", backref='supercat')  # 小分类外键关系关联
    goods = db.relationship("Goods", backref='supercat')  # 商品外键关系关联
...
class Subcat(db.Model):
    ...
    cat_name = db.Column(db.String(100))  # 小分类名称
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    super_cat_id = db.Column(db.Integer, db.ForeignKey("supercat.id"))  # 所属大分类
    goods = db.relationship("Goods", backref='subcat')  # 商品外键关系关联
