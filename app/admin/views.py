...
from app.admin.forms import GoodsForm, LoginForm
from app.models import Goods, SuperCat, Subcat, Admin, User, Orders, OrdersDetail
...
def index():
    ...
    page_data = Goods.query.order_by(
        Goods.addtime.desc()
    ).paginate(page=page, per_page=10)  # 分页 每页展示10条
...
def goods_del(id=None):
    """
    删除商品
    :param id: 根据goods_id做商品筛选,进行删除
    :return: index.html
    """
    goods = Goods.query.get_or_404(id)  # 根据goods_id做商品筛选
...
def login():
    ...
    if form.validate_on_submit():  # 验证提交表单
        ...
        # 密码错误时check_password,则此时not check_password(data[['pwd']) 为真
        if not admin.check_password(data['password']):
            flash('密码错误!', "err")  # 闪存错误信息
            return redirect(url_for("admin.login"))  # 跳转到后台登录页
        # 如果正确的,就要定义session的会话进行保存
...
@admin.route("/user/list/", method=['GET'])
@admin_login
def user_list():
    """
    会员列表
    ...
    if keyword:
        # 根据姓名或者邮箱查询
        filters = or_(User.username == keyword, User.email == keyword)  # 或
        page_data = User.query.filter(filters).order_by(User.addtime.desc()).paginate(page=page, per_page=5)
    else:
...
def orders_list():
    ...
    订单列表页面
    """
    keywords = request.args.get('keyword', '', type=str)
    page = request.args.get('page', 1, type=int)  # 获取page参数值
    if keywords:
        page_data = Orders.query.filter_by(id=keywords).order_by(Orders.addtime.desc()).paginate(page=page, per_page=10)
    else:
        page_data = Orders.query.filter_by(Orders.addtime.desc()).paginate(page=page, per_page=10)
